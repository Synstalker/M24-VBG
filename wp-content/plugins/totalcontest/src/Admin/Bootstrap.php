...
class Bootstrap
{
    ...
    public function __construct(Request $request, Environment $env)
    {
        ...
        $this->messages = [
            'reset-contest-success' => [__('Contest has been reset', 'totalcontest'), 'updated'],
            'reset-contest-error'   => [__('Contest reset failed', 'totalcontest'), 'error']
        ];
        ...
        if(Arrays::getDotNotation($this->onboarding, 'status', 'init') === 'init' && current_user_can('manage_options')) {
            $this->pages['onboarding'] = [
                'title'      => __( 'Onboarding', 'totalcontest' ),
                'name'       => __( 'Onboarding', 'totalcontest' ),
                'capability' => 'manage_options',
            ];
        }
        ...
        add_action('admin_post_reset_contest', [$this, 'resetContest']);
    ...
    public function menu()
    {
        ...
        if (current_user_can('manage_categories')):
            $GLOBALS['submenu'][$slug][] = [
                __('Categories', 'totalcontest'),
                'edit_posts',
                'edit-tags.php?taxonomy=' . TC_SUBMISSION_CATEGORY_TAX_NAME . '&amp;post_type=' . TC_CONTEST_CPT_NAME,
            ];
        endif;
        

        foreach ($this->pages as $pageSlug => $page):
            add_submenu_page($slug, $page['title'], $page['name'], empty($page['capability']) ? 'manage_options' : $page['capability'], $pageSlug, [$this, 'page']);
        endforeach;
    ...
    public function registerAssets()
    {
        ...
        wp_register_script('totalcontest-admin-contest-editor',
            ...
            ['jquery-datetimepicker', 'wp-color-picker', 'angular', 'angular-dnd-lists', 'angular-resource'],
            $assetsVersion);
        ...
        wp_register_script('totalcontest-admin-onboarding', "{$baseUrl}assets/dist/scripts/onboarding.js", ['angular', 'jquery', 'wp-util', 'angular-resource'], $assetsVersion);
