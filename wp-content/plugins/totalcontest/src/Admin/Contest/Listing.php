...
class Listing
{
    ...
    public function __construct(LogRepository $logRepository, SubmissionsRepository $submissionsRepository)
    {
        ...
        Tracking::trackScreens('contests');
    }
    ...
    public function actions($actions, $post)
    {
        ...
        if (current_user_can('edit_contest_submissions')):
            $actions['submissions'] = sprintf('<a href="%s">%s</a>', esc_attr(admin_url("edit.php?post_type={$submissionPostType}&contest={$post->ID}")),
                esc_html(__('Submissions', 'totalcontest')));
        endif;

        if (current_user_can('manage_options')):
            $url = admin_url("admin-post.php?action=reset_contest&post_type={$contestPostType}&contest={$post->ID}");
            $url = add_query_arg('_wpnonce', wp_create_nonce('reset_contest'), $url);
	        $confirm          = sprintf( "return prompt('%s') === 'confirm';", esc_js( __( 'Are you sure? This will remove submissions, votes and log from contest. Please type "confirm" to continue', 'totalcontest' ) ) );
            $actions['reset'] = sprintf('<a href="%s" style="color: #a00" onclick="%s">%s</a>', esc_attr($url), $confirm, esc_html(__('Reset contest', 'totalcontest')));
        endif;

        if (current_user_can('manage_options')):
            $actions['log'] = sprintf('<a href="%s">%s</a>', esc_attr(admin_url("edit.php?post_type={$contestPostType}&page=log&contest={$post->ID}")),
                esc_html(__('Log', 'totalcontest')));
        endif;
