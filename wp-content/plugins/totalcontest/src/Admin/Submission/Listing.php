...
class Listing
{
    ...
    public function __construct(
        ...
        Tracking::trackScreens('submissions');
    }
    ...
    public function filterByContest()
    {
        ...
        if ($post_type === TC_SUBMISSION_CPT_NAME) {
            ...
            ?>
            <select name="contest">
                <option value="0"><?php _e('Select Contest', 'totalcontest') ?></option>
    ...
    public function actions($actions, $post)
    {
        ...
        if ($post->post_status != 'publish' && current_user_can('publish_contest_submissions', $post->ID,
                $post->post_parent)):
            $actions = [
                    'confirm' => sprintf('<a href="%s" target="_blank">%s</a>',
                        esc_attr(admin_url("admin-ajax.php?action=totalcontest_contests_approve_submission&submission={$post->ID}")),
                        esc_html(__('Approve', 'totalcontest')))
                ] + $actions;
        endif;

        $actions['contest'] = sprintf('<a href="%s">%s</a>',
            esc_attr(admin_url("post.php?post={$post->post_parent}&action=edit")),
            esc_html(__('Contest', 'totalcontest')));
    ...
    public function download()
    {
        ...
        foreach ($fields as $field):
            $export->addColumn(new TextColumn(__('Form: ',
