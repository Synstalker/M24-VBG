...
class Model implements ModelContract {
	...
	public function __construct( $attributes ) {
		...
		$frequencyArgs['message']   = __( 'You cannot submit new entries in this contest.', 'totalcontest' );
	...
	public function getUrl( $args = [] ) {
		...
		$base = Misc::isDoingAjax() ? wp_get_referer() : add_query_arg( $queryParameters, home_url( $GLOBALS['wp']->request ) );
		...
		if ( $prettyPermalinks ):
			...
			$basePath   = preg_replace( '#(landing|participate|submissions|submission|content)/(.*)/?#i', '', empty( $currentUrl['path'] ) ? '/' : $currentUrl['path'] );
