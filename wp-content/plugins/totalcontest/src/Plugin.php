...
class Plugin extends \TotalContestVendors\TotalCore\Foundation\Plugin
{
    ...
    {
        // Bootstrap
        $this->container->share('bootstrap',
            function () {
                return new Bootstrap();
            });

        $this->container->share('admin.privacy',
            function () {
                return new Admin\Privacy\Policy($this->container->get('env'), $this->container->get('log.repository'));
            });

        // Schema migration
        $this->container->share('migrations.schema',
            function () {
        ...
        // Contests migration
        $this->container->share('migrations.totalcontest',
            function () {
                return new Migrations\Contest\TotalContest\Migrator($this->container->get('env'));
            });

        $this->container->share('migrations.migrators',
            function () {
                return [
        ...
        $this->container->share('modules.repository',
            function () {
        ...
        // Contest post type
        $this->container->share('contests.cpt',
            function () {
                return new Contest\PostType();
            });

        // Contest post defaults
        $this->container->share('contests.defaults',
            function () {
                        ...
                        'contest'       => [
                            ...
                            'submissions' => [
                                ...
                                'blocks'                                    => [
                                    'enabled'     => true,
                                    'submissions' => [
                                        ['uid' => Misc::generateUid(), 'type' => 'image', 'source' => '', 'fallback' => ''],
                                        [
                                            'uid'         => Misc::generateUid(),
                                            'type'        => 'title',
                                            'expressions' => [
                                                ['uid' => Misc::generateUid(), 'type' => 'val', 'source' => __('Submission', 'totalcontest') . ' #'],
                                                ['uid' => Misc::generateUid(), 'type' => 'var', 'source' => 'id'],
                                            ]
                                        ],
                                        [
                                            'uid'         => Misc::generateUid(),
                                            'type'        => 'subtitle',
                                            'expressions' => [
                                                ['uid' => Misc::generateUid(), 'type' => 'var', 'source' => 'votesWithLabel'],
                                                ['uid' => Misc::generateUid(), 'type' => 'val', 'source' => ' | '],
                                                ['uid' => Misc::generateUid(), 'type' => 'var', 'source' => 'viewsWithLabel'],
                                            ]
                                        ],
                                    ],
                                    'submission'  => [
        ...
        $this->container->add('contests.commands.send.notification',
            function ($contest, $submission) {
                return new Contest\Commands\SendNotification($contest, $submission, $this->container->get('http.request'));
            });

        // Submission post type
        $this->container->share('submissions.cpt',
            function () {
                return new Submission\PostType();
            });

        // Submission post type
        $this->container->share('submissions.repository',
            function () {
        ...
        $this->container->add('content.shortcode.image',
            function ($attributes, $content = null) {
                return new Shortcode\Image($attributes, $content);
            });

        $this->container->add('content.shortcode.video',
            function ($attributes, $content = null) {
                return new Shortcode\Video($attributes, $content);
            });

        $this->container->add('content.shortcode.audio',
            function ($attributes, $content = null) {
                return new Shortcode\Audio($attributes, $content);
            });

        $this->container->add('content.shortcode.text',
            function ($attributes, $content = null) {
        ...
        $this->container->share('admin.submission.editor',
            function () {
        ...
        // Structured data
        $this->container->share('decorators.structuredData',
            function () {
                return new Decorators\StructuredData();
            });

        // Helpers
        $this->container->share('url',
            function () {
