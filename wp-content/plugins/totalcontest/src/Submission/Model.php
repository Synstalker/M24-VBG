...
class Model implements ModelContract {
	...
	public function getSubtitle() {
		return wp_kses_post( Strings::template( $this->contest->getSettingsItem( 'contest.submissions.subtitle' ), $this->getBindings() ) );
	}
	...
	public function getShareAttributes() {
		 ...
		 * @since 2.0.0
		 */
		return apply_filters( 'totalcontest/filters/submission/share', array_intersect_key( $websitesUrls, $websites ), $this );
	}
	...
	public function getBlocks() {
		...
		foreach ( $blocks as &$block ):
			...
			if ( $block['type'] === 'text' || $block['type'] === 'title' || $block['type'] === 'subtitle' ):
				$block['html'] = Strings::template( $this->compileExpressions( $block['expressions'] ), $this->getBindings(), '' );
			elseif ( $block['type'] === 'image' ):
				if ( $block['source'] === '' ):
					$preview       = sprintf( '[totalcontest-image src="%s"]', TotalContest()->env( 'url' ) . 'assets/dist/images/no-preview.png' );
				...
				elseif ( $block['source'] === 'custom' ):
					...
					$preview           = sprintf( '[totalcontest-image src="%s"]', $this->getAttribute( $block['source'] ) ?: $block['fallback'] );
					$preview           = do_shortcode( $preview );
			...
			elseif ( $block['type'] === 'raw' ):
				$html          = Strings::template( $this->compileExpressions( $block['expressions'] ), $this->getBindings(), '' );
